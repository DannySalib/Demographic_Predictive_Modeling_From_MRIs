...
class MRI(Nifti1Image, ABC):   
    def __init__(self, dataobj, affine, header=None, extra=None, file_map=None, dtype=None):
        ...
        self._current_roi: ROIType = None
    ...
    def get_roi_fdata(self, roi: ROIType) -> np.ndarray:
        ...
        if self.dimension is MRIDimension.THREE_D:
            return img_data * self.current_roi_mask_fdata
        
        img_data_dimension = len(img_data.shape)
        assert img_data_dimension == MRIDimension.FOUR_D.value
    ...
    def get_roi_img(self, roi: ROIType) -> Nifti1Image:
    ...
    def show(self, nrows: int = 5, ncols: int = 5, at_time: int = 0) -> None:
        ...
        img_data_dimension = len(img_data.shape)
        if img_data_dimension != MRIDimension.FOUR_D.value:
